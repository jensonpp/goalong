...
Production: https://goalong.pages.dev (Cloudflare Worker, Git-connected, auto-deploys on push to `main`)
...
| Ride object + localStorage | ✅ | `RideRepository.createRide/getRides`, key `goalong.rides` |
...
| `goalong.pages.dev` URL | Low | Currently deployed as Worker → `*.workers.dev`. If `pages.dev` needed: recreate as Pages project (same build settings) |
